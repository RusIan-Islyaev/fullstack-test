let common = {
    ...
    user_edit_update: (user_id = 0) => {
        ...
        request({location: location, data: data}, (result) => {
            if (result.success === false && result.errors) {
                ...
                common.show_form_errors(result.errors);
            } else {
                common.modal_hide();
                html('table', result.html);
            }
    ...
    show_form_errors: (errors) => {
        // Clear ALL errors first (просто и эффективно)
        let allErrors = document.querySelectorAll('.field-error');
        for (let i = 0; i < allErrors.length; i++) {
            allErrors[i].remove();
        }
        
        let allErrorInputs = document.querySelectorAll('.input_error');
        for (let i = 0; i < allErrorInputs.length; i++) {
            allErrorInputs[i].classList.remove('input_error');
        }
        ...
        if (firstErrorField) {
            let firstInput = document.getElementById(firstErrorField);
            if (firstInput) firstInput.focus();
        }
    },

    fields_check : () => {
       const fields = ['first_name', 'last_name', 'phone', 'email', 'plot_ids'];
        ...
        fields.forEach(fieldId => {
            ...
            if (input) {
                input.addEventListener('input', function(e) {
                    ...
                    const inputGroup = this.closest('.input_group_modal');
                    if (inputGroup) {
